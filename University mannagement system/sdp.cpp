...
for (i = 0; i < 3; i++) {
  ...
  if (strcmp(username, "Pandacoders") == 0 && strcmp(password, "Raihan") == 0) {
    cout<<endl;
    loading();
    break;
  } else {
...
void MainMenu(void)
{
    ...
    system("cls");
    title();
  int ch = 0; // Initialize ch to a non-2 value to enter the loop.

    while (ch != 3)
    {
    cout << "\n\n\n\n\t\t\t\t1. User"<<endl;
    cout << "\t\t\t\t2. University admin"<<endl;
    cout << "\t\t\t\t0. Logout..."<<endl<<endl<<endl;
...
void User(void)
{
    ...
    int choose=0;
    title();
     // Initialize choose to a non-10 value to enter the loop.
    while (choose != 11) {
    cout << "\n\n\n\t\t\t\t\t\t\t     <USER>"<<endl;
    cout << "\n\n\t\t\t\t1. Faculty Member"<<endl;
    cout << "\t\t\t\t2. Student"<<endl;
    cout << "\t\t\t\t3. Teacher Assistant"<<endl;
    cout << "\t\t\t\t4. Employees"<<endl;
    cout << "\t\t\t\t5. Graduated"<<endl;
    cout << "\t\t\t\t6. Library"<<endl;
    cout << "\t\t\t\t7. Hostel"<<endl;
    cout << "\t\t\t\t8. Transport"<<endl;
    cout << "\t\t\t\t9. Club"<<endl;
    cout << "\t\t\t\t10. Admission Panel"<<endl;
    cout << "\t\t\t\t0. Main-menu"<<endl<<endl<<endl;
...
void Un_ad(void)
{
    ...
    system("COLOR 0B");
    title();
    int choose = 0; // Initialize choose to a non-10 value to enter the loop.

    while (choose != 10) {
    cout << "\n\n\n\t\t\t\t\t\t       <University Admin>\n";
    cout << "\n\n\n\t\t\t\t1. Faculty Member Record\n";
    cout << "\t\t\t\t2. Student Record\n";
    cout << "\t\t\t\t3. Teacher Assistant Record\n";
    cout << "\t\t\t\t4. Employees Record\n";
    cout << "\t\t\t\t5. Graduated Record\n";
    cout << "\t\t\t\t6. Library Record\n";
    cout << "\t\t\t\t7. Hostel Record\n";
    cout << "\t\t\t\t8. Transport Record\n";
    cout << "\t\t\t\t9. Club\n";
    cout << "\t\t\t\t10. Admission Panel Record\n";
    cout << "\t\t\t\t0. Logout from admin...\n\n\n";
    cout << "\t\t\t\tChoose from 1 to 10 :=>";
    cin >> choose;
...
void Fa(void)
{
    ...
    cout << "\t\t\t\t2. Faculty Display\n";
    cout << "\t\t\t\t3. Faculty Search\n";
    cout << "\t\t\t\t4. Faculty Delete\n";
    cout << "\t\t\t\t5. Back\n";
    cout << "\n\t\t\t\tEnter your choice => ";
    cin >> ch;
...
void fa_display_record(void) {
  ...
  for (int i = 0; i < feof(fa); i++) {
    fscanf(fa, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.position,
           &p.email, &p.mobile, &p.blood, &p.address, &p.salary);
...
void fa_serch_record(void) {
  ...
  for (int i = 0; i < feof(fa); i++) {
    ...
    if (p.id == faid) {
      printf("\n\n%s\t%d\t\%s\t%s\t%s\t%s\t%s\t%s\n", p.name, p.id,
             p.position, p.email, p.mobile, p.blood, p.address, p.salary);
      flag = 1;
      break;
    }
  }
...
void fa_del_record(void)
{
    ...
    for (;;)
    {
        int result = fscanf(fa, "%s %d %s %s %s %s %s %s", &p.name, &p.id, &p.position, &p.email, &p.mobile, &p.blood, &p.address, &p.salary);
...
void st_serch_record(void)
{
    ...
    printf("\n\n\t\t\t\tEnter an ID which you want to search: ");
    scanf("%d", &faid);
...
void St_registration()
{
    ...
    for (;;)
    {
        ...
        printf("\t\t\t\t3.Course Search\n");
        printf("\t\t\t\t4.Course Delete\n");
        printf("\t\t\t\t5.Back\n");
        printf("\n\t\t\t\tEnter your choice => ");
        scanf("%d", &ch);
...
void stre_display_record(void)
{
    ...
    for (;;)
    {
        if (fscanf(stre, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.cou_1re, &p.cou_2re, &p.cou_3re, &p.cou_4re, &p.cou_5re, &p.cou_6re) == EOF)
            break;
...
void stre_del_record(void)
{
    ...
    printf("\n\t\tPrevious stored data*****");
    stre_display_record();
    printf("\n\n\t\t!!!!!!Enter the name number you want to delete =>");
    scanf("%s", &name);
...
void py_display_record(void)
{
    ...
    for (;;)
    {
        if (fscanf(py, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.mobile, &p.subject, &p.intake_section, &p.number_semister, &p.number_installment, &p.amount) == EOF)
            break;
...
void strg_login()
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            while (1)  // Converted inner while loop to a for loop
            {
                if (feof(strg))
                {
                    break;  // Exit the inner loop when the end of file is reached
                }
...
void s_lib()
{
    ...
    int ch = -1; // Initialize 'ch' to a value that doesn't match any case
...
void em_display_record(void)
{
    ...
    for (;;) {
    int result = fscanf(em, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.position, &p.email, &p.mobile, &p.blood, &p.address, &p.salary);
    if (result != 8) {
        // Assuming 8 fields were successfully scanned, change this number if necessary
        break;  // Exit the loop when reading fails
    }
    printf("%s\t%d\t\%s\t%s\t%s\t%s\t\t%s  %s\n", p.name, p.id, p.position, p.email, p.mobile, p.blood, p.address, p.salary);
}
...
void em_search_record(void)
{
    ...
    if (p.id == emid) {
        printf("\n\n%s\t%d\t\%s\t%s\t%s\t%s\t%s\t%s\n", p.name, p.id, p.position, p.email, p.mobile, p.blood, p.address, p.salary);
        flag = 1;
        break;  // Exit the loop when the desired record is found
    }
}
...
void em_del_record(void)
{
    ...
    for (;;) {
    int result = fscanf(em, "%s %d %s %s %s %s %s %s", &p.name, &p.id, &p.position, &p.email, &p.mobile, &p.blood, &p.address, &p.salary);
    if (result != 8) {
        // Assuming 8 fields were successfully scanned, change this number if necessary
        break;  // Exit the loop when reading fails
    }
...
void gra(void)
{
   ...
   for (int ch = -1; ch != 3;) {
    ...
    if (ch != 3) {
        printf("Press any key to continue............... ");
        // Add code here to wait for user input before continuing
    }
}
...
void lib_login()
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            cout<< "\n\t\t\t\tPassword : ";
            cin>>id2;
           for (int i = 0; i < feof(li); i++) {
  fscanf(li, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.intake_section,
         &p.mobile, &p.book_receive_date, &p.book_submit_date, &p.book_name,
         &p.total_book);
...
void hpy_display_record(void)
{
    ...
    for (; !feof(hpy);) {
    fscanf(hpy, "%s %d %s %s %s %s %s %s", &p.name, &p.id, &p.gender, &p.mobile, &p.subject, &p.intake_section, &p.payment_date, &p.amount);
...
void hol_login() {
  ...
  while (true) {
    ...
    for (int i = 0; i < feof(ho); i++) {
      fscanf(ho, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.gender,
             &p.email, &p.mobile, &p.blood, &p.address, &p.room_n);
...
void Tr(void)
{
    system("cls");
    title();
    int ch = -1;  // Initialize ch to -1 to ensure the loop runs at least once
...
void tra_login()
{
    ...
    while(true)
    {
        ...
        else
        {
            ...
            cout<< "\n\t\t\t\tPassword : ";
            cin>>id2;
           for (int i = 0; i < feof(tr); i++) {
  fscanf(tr, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.intake_section,
         &p.subject, &p.mobile, &p.blood, &p.address, &p.bus_name);
...
void admimssion_login() {
  ...
  for (i = 0; i < 7; i++) {
    ...
    password[j] = '\0';
    printf("\n");
    if (strcmp(username, "Raihan") == 0 && strcmp(password, "Ehsan") == 0) {
      printf("\n");
      loading();
      break;
    } else {
...
void ad_display_record(void)
{
    ...
    for (;;) {
    if (fscanf(ad, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.program, &p.email, &p.mobile, &p.blood, &p.address, &p.intake_section) != 8) {
        break; // Exit the loop when reading fails (end of file or invalid format)
    }
...
void ad_serch_record(void)
{
    ...
    printf("\n\n\t\t\t\tEnter a id which you want to search :");
    scanf("%d",&adid);
    for (int i = 0; i < feof(ad); i++) {
  fscanf(ad, "%s %d %s %s %s %s %s %s\n", &p.name, &p.id, &p.program,
          &p.email, &p.mobile, &p.blood, &p.address, &p.intake_section);
